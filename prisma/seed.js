...
async function main() {
    ...
    for (let num = 1; num <= 4; num++) {
      ...
      const depto = await prisma.departamento.upsert({
        ...
        create: {
          ...
          piso,
          torre: num <= 2 ? 'A' : 'B',
          tipo: piso === 5 ? 'PENTHOUSE' : 'APARTAMENTO',
          metrosCuadrados: piso === 5 ? 120 : 80,
          habitaciones: piso === 5 ? 4 : 3,
          banos: piso === 5 ? 3 : 2,
  ...
  for (let i = 0; i < nombres.length; i++) {
    const r = await prisma.residente.upsert({
      ...
      create: {
        ...nombres[i],
        email: `${nombres[i].nombre.toLowerCase()}@email.com`,
        telefono: `0414-${String(1000000 + i).slice(0, 7)}`,
        tipo: i % 3 === 0 ? 'INQUILINO' : 'PROPIETARIO',
        departamentoId: deptos[i]?.id,
      },
    });
  ...
  for (const res of residentesList.slice(0, 8)) {
    for (const mes of meses) {
      await prisma.pago.create({
        data: {
          monto: 150 + Math.floor(Math.random() * 50),
          mesPago: mes,
          metodoPago: metodos[Math.floor(Math.random() * metodos.length)],
          referencia: `REF-${Date.now()}-${pagoCount}`,
          estado: Math.random() > 0.3 ? 'VERIFICADO' : 'PENDIENTE',
          residenteId: res.id,
          departamentoId: deptos[residentesList.indexOf(res)]?.id || deptos[0].id,
        },
      });
      pagoCount++;
    }
  ...
  for (const g of gastosData) {
    await prisma.gastoComun.create({ data: { ...g, fecha: new Date() } });
  }
  console.log(`✅ ${gastosData.length} gastos creados`);

  // Anuncios
  const anunciosData = [
    { titulo: 'Mantenimiento del ascensor', contenido: 'Se realizará mantenimiento preventivo del ascensor el próximo sábado de 8am a 2pm. Por favor usar las escaleras durante ese horario.', prioridad: 'ALTA' },
    { titulo: 'Reunión de condominio', contenido: 'Se convoca a todos los propietarios a la reunión ordinaria de condominio el día viernes a las 7pm en el salón de eventos.', prioridad: 'NORMAL' },
    { titulo: 'Nuevo horario de vigilancia', contenido: 'A partir del próximo mes, la vigilancia nocturna será de 6pm a 6am. Se solicita a todos los residentes colaborar con la identificación al ingresar.', prioridad: 'NORMAL' },
  ];

  for (const a of anunciosData) {
    await prisma.anuncio.create({ data: { ...a, autorId: admin.id } });
  }
  console.log(`✅ ${anunciosData.length} anuncios creados`);

  // Mantenimiento
  const mantData = [
    { titulo: 'Fuga en tubería piso 3', descripcion: 'Se detectó una fuga de agua en la tubería principal del piso 3 que está afectando al departamento 301.', categoria: 'PLOMERIA', prioridad: 'URGENTE', estado: 'EN_PROGRESO' },
    { titulo: 'Bombillo fundido pasillo piso 2', descripcion: 'El bombillo del pasillo del segundo piso se fundió y necesita reemplazo.', categoria: 'ELECTRICIDAD', prioridad: 'BAJA', estado: 'PENDIENTE' },
    { titulo: 'Pintura fachada principal', descripcion: 'La fachada principal del edificio necesita una mano de pintura. Se cotizará con varios proveedores.', categoria: 'PINTURA', prioridad: 'NORMAL', estado: 'PENDIENTE', costoEstimado: 3500 },
  ];
  ...
  for (let i = 0; i < mantData.length; i++) {
    await prisma.mantenimiento.create({
      data: {
        ...mantData[i],
        departamentoId: deptos[i]?.id,
        residenteId: residentesList[i]?.id,
      },
    });
  }
  console.log(`✅ ${mantData.length} solicitudes de mantenimiento creadas`);
