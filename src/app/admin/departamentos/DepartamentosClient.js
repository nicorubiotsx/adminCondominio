...
import { createDepartamento, deleteDepartamento } from '@/actions/departamentos';
...
export default function DepartamentosClient({ departamentos, total, pages, currentPage, search }) {
  ...
  const [state, formAction, pending] = useActionState(createDepartamento, { errors: {}, success: false });
        ...
        {departamentos.length > 0 ? (
          <>
            <table className="data-table">
              <thead>
                <tr><th>Número</th><th>Piso</th><th>Torre</th><th>Tipo</th><th>M²</th><th>Hab.</th><th>Estado</th><th>Residentes</th><th>Acciones</th></tr>
              </thead>
              <tbody>
                {departamentos.map((d) => (
                  <tr key={d.id}>
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>➕ Nuevo Departamento</h2><button className="modal-close" onClick={() => setShowModal(false)}>✕</button></div>
            <form action={formAction}>
              <div className="modal-body">
                <div className="form-row">
                  <div className="form-group">
                    <label className="form-label">Número *</label>
                    <input name="numero" className="form-input" placeholder="101" required />
                    {state.errors?.numero && <p className="form-error">{state.errors.numero}</p>}
                  </div>
                  <div className="form-group">
                    <label className="form-label">Piso *</label>
                    <input name="piso" type="number" className="form-input" placeholder="1" required />
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input name="torre" className="form-input" placeholder="A" />
                  </div>
                  <div className="form-group">
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input name="metrosCuadrados" type="number" step="0.01" className="form-input" placeholder="80" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Alícuota %</label>
                    <input name="alicuota" type="number" step="0.01" className="form-input" placeholder="2.5" defaultValue="0" />
                  </div>
                </div>
                <div className="form-row">
                  <div className="form-group">
                    <label className="form-label">Habitaciones</label>
                    <input name="habitaciones" type="number" className="form-input" placeholder="3" />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Baños</label>
                    <input name="banos" type="number" className="form-input" placeholder="2" />
                  </div>
                </div>
                <div className="form-row">
                  <div className="form-group">
