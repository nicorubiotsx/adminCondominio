...
import { createMantenimiento, updateMantenimientoEstado, deleteMantenimiento } from '@/actions/mantenimiento';
                  ...
                  <tr key={m.id}>
                    ...
                    <td>
                      <div className="action-btns">
                        {m.estado === 'PENDIENTE' && (
                          <button className="btn btn-success btn-sm" onClick={() => handleUpdateEstado(m.id, 'EN_PROGRESO')} title="Iniciar">▶️</button>
                        )}
                        {m.estado === 'EN_PROGRESO' && (
                          <button className="btn btn-success btn-sm" onClick={() => handleUpdateEstado(m.id, 'COMPLETADO')} title="Completar">✅</button>
                        )}
                        {(m.estado === 'PENDIENTE' || m.estado === 'EN_PROGRESO') && (
                          <button className="btn btn-warning btn-sm" onClick={() => handleUpdateEstado(m.id, 'CANCELADO')} title="Cancelar">⛔</button>
                        )}
      ...
      {showModal && (
        <div className="modal-overlay" onClick={() => setShowModal(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header"><h2>➕ Nueva Solicitud</h2><button className="modal-close" onClick={() => setShowModal(false)}>✕</button></div>
            <form action={formAction}>
              <div className="modal-body">
                <div className="form-group">
                  <label className="form-label">Título *</label>
                  <input name="titulo" className="form-input" placeholder="Ej: Fuga en baño del piso 3" required />
                  {state.errors?.titulo && <p className="form-error">{state.errors.titulo}</p>}
                </div>
                <div className="form-group">
                  <label className="form-label">Descripción *</label>
                  <textarea name="descripcion" className="form-textarea" placeholder="Describe el problema en detalle..." required />
                ...
                <div className="form-row">
                  ...
                  <div className="form-group">
                    ...
                    <select name="residenteId" className="form-select">
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <input name="costoEstimado" type="number" step="0.01" className="form-input" placeholder="0.00" />
                  </div>
                </div>
                <div className="form-group">
                  <label className="form-label">Notas</label>
                  <textarea name="notas" className="form-textarea" placeholder="Notas adicionales..." />
                </div>
              </div>
              <div className="modal-footer">
                <button type="button" className="btn btn-secondary" onClick={() => setShowModal(false)}>Cancelar</button>
                <button type="submit" className="btn btn-primary" disabled={pending}>{pending ? '⏳ Guardando...' : '💾 Crear Solicitud'}</button>
              </div>
            </form>
          </div>
        </div>
